...
object WikipediaRanking {
  ...
  val conf: SparkConf =
    new SparkConf().setMaster("local[2]").setAppName("CountingSheep")
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = {
    def langCountAccumulator(accumulator: Int,
                             article: WikipediaArticle): Int = {
    ...
    rdd.aggregate(0)(langCountAccumulator, _ + _)
  }
  ...
  def rankLangsUsingIndex(
                           index: RDD[(String, Iterable[WikipediaArticle])]): List[(String, Int)] = {
    index
